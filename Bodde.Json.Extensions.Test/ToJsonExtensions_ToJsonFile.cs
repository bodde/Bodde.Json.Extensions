...
public class ToJsonExtensions_ToJsonFile
{
    [Fact]
    public void NonNull_Writes_Json_To_File()
    {
        var filename = Path.Combine(Path.GetTempPath(), $"{Guid.NewGuid()}.json");

        try
        {
            ModelBuilder.CreateJohnDoe().ToJsonFile(filename);

            var actual = File.ReadAllText(filename);

            Assert.NotEmpty(actual);
            Assert.Contains("\"FirstName\":\"John\"", actual);
        }
        finally
        {
    ...
    public void Null_Object_Throws_ArgumentNullException()
    {
        ...
        Assert.Throws<ArgumentNullException>(() => sut!.ToJsonFile(filename));
    }
    ...
    public void Null_Filename_Throws_ArgumentNullException()
    {
        Assert.Throws<ArgumentNullException>(
            () => ModelBuilder.CreateJohnDoe().ToJsonFile(null!));
    }
    ...
    public void Empty_Filename_Throws_ArgumentException()
    {
        Assert.Throws<ArgumentException>(
            () => ModelBuilder.CreateJohnDoe().ToJsonFile(string.Empty));
    }
}